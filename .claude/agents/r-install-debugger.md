...
3. **If the env isn't set up** (r_install.sh says R / R_LIBS_USER is missing), tell the facilitator to run `r_env.sh R/<version> <workspace>` first — do not try to set it up yourself.
   - **If `r_install.sh` refuses because you're on a login node** (no `$NSLOTS`/`$JOB_ID`), do **not** work around it or set `ALLOW_LOGIN_NODE=1`. Report back that the reproduction needs an interactive job and suggest requesting one (e.g. `qrsh -P <project> -pe omp 4`), then re-running from inside it.
